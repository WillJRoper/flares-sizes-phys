...
def plot_stellar_hmr(stellar_data, snap, weight_norm, cut_on="hmr"):
    ...
    okinds = np.logical_and(hdf["galaxy_data"]["nstar"][:] > 100,
                            hdf["galaxy_data"]["ngas"][:] > 100)
    ...
    plot_meidan_stat(tng_ms, tng_rs, np.ones(tng_rs.size),
                     ax, "Illutris-TNG 300", "deepskyblue", bin_edges)
...
def plot_weighted_gas_size_mass(snap, regions, weight_norm, ini_path):
    ...
    for reg in regions:
        ...
        for igal in range(mass.size):
            ...
            if g_hmr / hmr > 10:
                print("Large ratio galaxy:", g_hmr, hmr, m, g, sg)
