...
def plot_ssfr_mass_size_change(stellar_data, gas_data, snaps, weight_norm):
    ...
    ax1.set_title(
        "$R_{1/2,\star}^{A} > 1 \ \mathrm{pkpc} \ \cup \ R_{1/2,\star}^{B} > 1 \ \mathrm{pkpc}$")
    okinds = np.logical_and(tot_hmrs <= 1, tot_prog_hmrs > 1)
    im = ax2.hexbin(delta_hmr[okinds], tot_ssfr[okinds],  gridsize=50,
                    ...
                    linewidths=0.2, cmap="plasma", extent=extent)
    ax2.set_title(
        "$R_{1/2,\star}^{A} > 1 \ \mathrm{pkpc} \ \cup \ R_{1/2,\star}^{B} \leq 1 \ \mathrm{pkpc}$")
    okinds = np.logical_and(tot_hmrs <= 1, tot_prog_hmrs <= 1)
    im = ax3.hexbin(delta_hmr[okinds], tot_ssfr[okinds],  gridsize=50,
                    ...
                    linewidths=0.2, cmap="plasma", extent=extent)
    ax3.set_title(
        "$R_{1/2,\star}^{A} \leq 1 \ \mathrm{pkpc} \ \cup \ R_{1/2,\star}^{B} \leq 1 \ \mathrm{pkpc}$")
